...
class Coarse(nn.Module):
    def __init__(self, opt):
        ...
        self.coarse1 = nn.Sequential(
            # encoder
            GatedConv2d(4, 32, 5, 2, 2, activation=opt.activation,
                        norm=opt.norm, sc=True),
            GatedConv2d(32, 32, 3, 1, 1, activation=opt.activation,
                        norm=opt.norm, sc=True),
            GatedConv2d(32, 64, 3, 2, 1, activation=opt.activation,
                        norm=opt.norm, sc=True)
        )
        self.coarse2 = nn.Sequential(
            GatedConv2d(64, 64, 3, 1, 1, activation=opt.activation,
                        norm=opt.norm, sc=True),
            GatedConv2d(64, 64, 3, 1, 1, activation=opt.activation,
                        norm=opt.norm, sc=True),
            GatedConv2d(64, 64, 3, 1, 1, activation=opt.activation,
        ...
        self.coarse4 = nn.Sequential(
            GatedConv2d(64, 64, 3, 1, 2, dilation=2,
                        activation=opt.activation, norm=opt.norm, sc=True),
            GatedConv2d(64, 64, 3, 1, 2, dilation=2,
                        activation=opt.activation, norm=opt.norm, sc=True),
            GatedConv2d(64, 64, 3, 1, 2, dilation=2,
                        activation=opt.activation, norm=opt.norm, sc=True)
...
class GatedGenerator(nn.Module):
    def __init__(self, opt):
        ...
        self.refinement1 = nn.Sequential(
            GatedConv2d(3, 32, 5, 2, 2, activation=opt.activation,
                        norm=opt.norm),  # [B,32,256,256]
        ...
        self.refinement7 = nn.Sequential(
            GatedConv2d(256, 128, 3, 1, 1,
                        activation=opt.activation, norm=opt.norm),
            TransposeGatedConv2d(
                128, 64, 3, 1, 1, activation=opt.activation, norm=opt.norm),
            GatedConv2d(64, 64, 3, 1, 1,
                        activation=opt.activation, norm=opt.norm)
        )
        ...
        self.refinement9 = nn.Sequential(
            TransposeGatedConv2d(
                64, 32, 3, 1, 1, activation=opt.activation, norm=opt.norm),
...
class PatchDiscriminator(nn.Module):
    def __init__(self, opt):
        ...
        self.block1 = Conv2dLayer(
            4, 64, 3, 2, 1, activation='lrelu', norm=self.norm, sn=self.sn)
        self.block2 = Conv2dLayer(
            64, 128, 3, 2, 1, activation='lrelu', norm=self.norm, sn=self.sn)
        self.block3 = Conv2dLayer(
            128, 256, 3, 2, 1, activation='lrelu', norm=self.norm, sn=self.sn)
        self.block4 = Conv2dLayer(
            256, 256, 3, 2, 1, activation='lrelu', norm=self.norm, sn=self.sn)
        self.block5 = Conv2dLayer(
            256, 256, 3, 2, 1, activation='lrelu', norm=self.norm, sn=self.sn)
        self.block6 = Conv2dLayer(
            256, 16, 3, 2, 1, activation='lrelu', norm=self.norm, sn=self.sn)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--save_path', type=str,
                        default='./models', help='saving path that is a folder')
    ...
    parser.add_argument('--multi_gpu', type=bool,
                        default=False, help='nn.Parallel needs or not')
    ...
    parser.add_argument('--load_name', type=str,
                        default='', help='load model name')
    # Training parameters
    parser.add_argument('--epochs', type=int, default=40,
                        help='number of epochs of training')
    parser.add_argument('--batch_size', type=int,
                        default=6, help='size of the batches')
    ...
    parser.add_argument('--weight_decay', type=float,
                        default=0, help='Adam: weight decay')
    ...
    parser.add_argument('--lambda_l1', type=float,
                        default=100, help='the parameter of L1Loss')
    ...
    parser.add_argument('--latent_channels', type=int,
                        default=32, help='latent channels')
    parser.add_argument('--pad_type', type=str,
                        default='replicate', help='the padding type')
    ...
    parser.add_argument('--baseroot', type=str,
                        default='./dataset/val_256', help='the training folder')
    parser.add_argument('--mask_type', type=str,
                        default='free_form', help='mask type')
    parser.add_argument('--imgsize', type=int,
                        default=256, help='size of image')
    parser.add_argument('--margin', type=int, default=10,
                        help='margin of image')
    parser.add_argument('--mask_num', type=int,
                        default=15, help='number of mask')
    parser.add_argument('--bbox_shape', type=int, default=30,
                        help='margin of image for bbox mask')
    ...
    parser.add_argument('--max_width', type=int, default=30,
